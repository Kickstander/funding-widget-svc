...
async function seedPGData() {
  ...
  await db.query(`CREATE TABLE IF NOT EXISTS campaigns (id SERIAL PRIMARY KEY, campaign TEXT, author TEXT, user TEXT, country INT, pledgedAmount INT,
    goal INT, backers INT, endTime INT);`)
    .catch((err) => { 
      console.log(err);
    });
    
  await db.query(`COPY campaigns (campaign, author, user, country, pledgedAmount, goal, backers, endTime) FROM '${csvFile}' CSV HEADER;`)
    .catch((error) => { 
      console.log(error);
    });

  console.log(`SEEDED PG DATA: ${new Date().getTime() - currentTime} milliseconds`);
  ...
  for (let i = 0; i < 100; i++) {
    let csvFile = path.resolve(__dirname, `./pledgeData.csv`);
